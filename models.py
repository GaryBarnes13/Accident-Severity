...
bagging = BaggingClassifier(
    base_estimator=t, n_estimators=10, random_state=0
).fit(
    X_labeled, y
)  # score: 97%
...
logreg = LogisticRegression(random_state=0).fit(
    X_labeled, y
)  # stops and reaches iteration limit, but outputs 30%
...
neigh = KNeighborsClassifier(
    n_neighbors=3
)  # score: 71%, 3 neighbors is best cause less and more decrease accuracy.
...
xgb_model = XGBClassifier(
    max_depth=15
)  # score: 99% at depth = 15 likely over fitting
